...
char *appendmol_name(Lattice *lat, Adsorbate *ads, int c1, int c2)
{
...
int init_ads_direction(Lattice *lat, Adsorbate *ads, int id_from, int id_to)
{
    ...
    simd_double3 ads_stem_vec = ads->vtable->get_stem_vector(ads);
    /* Get the target direction vector */
    simd_double3 direction_vec =
        lat->mol->vtable->get_vector_ab(lat->mol, id_from, id_to);
    double stem_chain_angle = simd_vector_angle(ads_stem_vec, direction_vec);
    simd_double3 rotAxis =
        simd_normalize(simd_cross(ads_stem_vec, direction_vec));
    simd_quatd rotate_q = simd_quaternion(stem_chain_angle, rotAxis);
                            ...
                            char *pathName, double height)
{
    ...
    simd_double3 center_coords_of_carbon_sites = /* malloced */
        lat->mol->vtable->get_centroid_ab(lat->mol, carbon_1, carbon_2);
    simd_double3 center_coords_of_cd_sites =
        ads->mol->vtable->get_centroid_ab(ads->mol, cd_1, cd_2);
    simd_double4x4 transMat = translateMatrix(center_coords_of_cd_sites,
    ...
    /* Clean memory */
    char *newName = appendmol_name(lat, ads, c1, c2);
    Lattice *newModel =
